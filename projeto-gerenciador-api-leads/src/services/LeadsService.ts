...
export class LeadsService {
    ...
    async getLeadById(id: number) {
        ...
        if (!lead) throw new HttpError(400, "lead não encontrado!")
        return lead
    }
    ...
    async updateLead(leadId: number, params: Partial<CreateLeadAttributes>) {
        ...
        if (lead.status === "New" && params.status !== undefined && params.status !== LeadStatus.Contacted) {
    ...
    async deleteLead(leadId: number) {
        ...
        if (!leadExists) throw new HttpError(400, "lead não encontrado")
